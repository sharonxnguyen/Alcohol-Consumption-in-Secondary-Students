...
t <- model.matrix(G3~., data) # use student_por or data?
# 
train = sample(nrow(t), num_samp)
x.train = t[train, ]
y.train = data[train, ]$G3
# 
# # The rest as test data
x.test = t[-train, ]
y.test = data[-train, ]$G3
...
#Ytrain <- train$G3
#Xtrain <- train %>% select(-grade) %>% scale(center=TRUE, scale=TRUE)

#Ytest <- test$G3
#Xtest <- test %>% select(-grade) %>% scale(center=TRUE, scale=TRUE)

```

##### Matt's pile of trash
```{r}
set.seed(123)

# training classifier, predict on training set
...
```
This approach leads to correct predictions for 92% of the locations in the test set. In other words, the test error rate is 8%. This is really equivalent to:
...
```{r}
# Random Forest
data.rf <- randomForest(grade ~., data = data, subset = train.indices, norm.votes = FALSE)
print(data.rf)
